...
# combined = shigella_peptide_pST[ProteinName == "A0A0N4SW73_MOUSE|A0A0N4SW73_T886"]
...
summ_sh = MSstatsWeightedSummary::getWeightedProteinSummary(input_with_cls[Cluster == 5276], norm = "Huber", norm_parameter = 1e-3, max_iter = 100)
...
unique(comp$Protein)
unique(concat_summ$Protein)
ggplot(comp,
       aes(x = Channel, y = Abundance, group = Method, color = Method)) +
  geom_point() +
  geom_line() +
  facet_grid(Protein~Run) +
  theme_bw() +
...
ggplot(comp2,
  ...
  theme(legend.title = element_text(size = 14),
        ...
        legend.box = "vertical") +
  scale_color_manual(name = "method", values = sort(colors), labels = (full_names[meths[names(sort(colors))]])) +
  scale_linetype_discrete(name = "peptide") +
  ylab("log-intensity") +
  xlab("channel")
# ggsave("multisite_profiles.pdf", device = "pdf", width = 10, height = 5, units = "in", scale = 1)
